...
void CAN_modify_data(MOTOR_can_send *motor_s)
{
	...
	motor_s->motor_can_send_data.comd.k_pos  = (int16_t)(motor_s->K_P*1280.0f);//位置增益转换 (范围：0-25.6)
	motor_s->motor_can_send_data.comd.k_spd  = (int16_t)(motor_s->K_W*1280.0f);//速度增益转换 (范围：0-25.6)
	motor_s->motor_can_send_data.comd.pos_des  = (int32_t)(motor_s->Pos/6.2832f*32768);//位置转换 (单位：rad)
	motor_s->motor_can_send_data.comd.spd_des  = (int16_t)(motor_s->W/6.2832f*256);//速度转换 (单位：rad/s)
	motor_s->motor_can_send_data.comd.tor_des  = (int16_t)(motor_s->T*256);//力矩转换 (单位：N/m)
}
...
void unitree_CAN_Send(CAN_HandleTypeDef *hcanx,uint32_t moduleId,uint32_t ctrlmode,MOTOR_can_send *motor_s)
{
	...
	TxMessage.ExtId =(0|(moduleId<<27)|(((uint8_t *)(motor_s))[0]<<8)|(ctrlmode<<16));
	...
	switch(ctrlmode){
		...
		HAL_CAN_AddTxMessage(hcanx, &TxMessage,unitree_can_send_kkdata,&send_mail_box2);break;
	}
}
